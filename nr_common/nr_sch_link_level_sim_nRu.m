...
N_rru_dly=[0,0,50,100,150,200,250,300,50];
...
for n_slot = 0 : sim_dur_slots-1
    ...
    for i = 1:length(UE)
        ...
        size_y=size(y_tx_ue);
        y_tx_ru=zeros(size_y(1),8);
        if N_rru>1
            for ant=1:2
                for n_rru=1:N_rru
                    y_tx_ru(1+N_rru_dly(i):end,n_rru)=N_rru_coe(i)*y_tx_ue(1:end-N_rru_dly(n_rru),ant);
                end
                sum_coe=sum(N_rru_coe(1:N_rru));
                y_tx_ue(:,ant)=sum(y_tx_ru,2)/sum_coe;
            end
        end
