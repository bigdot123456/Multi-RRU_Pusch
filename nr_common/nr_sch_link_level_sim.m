...
%
%  N_rru         - RRU Number to combine, default is 1, should be positive integer
% Returns:
...
for n_slot = 0 : sim_dur_slots-1
    ...
    for i = 1:length(UE)
        if channel.rayleigh_en
            ...
            y_tx_ue = apply_fading_td(UE(i).y_tx, 0, frame_cfg.F_s, UE(i).pdp, channel.method, [UE(i).N_ant_TX, N_ant_eNB_RX, channel.MIMO_corr(1), channel.MIMO_corr(2)]);
            %        y_tx_ue = UE(i).y_tx;
        end
        
        % add multi-RRU delay
        [len,ant]=size(y_tx_ue);
        y_tx_ue_8rru=zeros(len,ant);
        
        % N_rru_dly=[0,25,50,100,150,200,250,300,50];
        % N_rru_coe=[1,0.5,0.5,0.5,0.27,0.26,0.2,0.1];
        for l=1:ant
            rru_8ch=zeros(len,length(N_rru_dly));
            for r=1:N_rru
                rru_8ch(1:len-N_rru_dly(r),r)=N_rru_coe(r)*y_tx_ue(1+N_rru_dly(r):end,l);
                y_tx_ue_8rru(:,l)=y_tx_ue_8rru(:,l)+rru_8ch(:,r);
            end
            
        end
        
        y_tx_ue=y_tx_ue_8rru;
